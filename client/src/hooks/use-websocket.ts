...
export function useWebSocket<T extends WebSocketMessage = WebSocketMessage>(
	...
	const [connectionStatus, setConnectionStatus] = useState<
		'connecting' | 'connected' | 'disconnected' | 'error' | 'reconnecting' | 'refreshing_token'
	>('disconnected')
		...
		try {
			...
			ws.current.onclose = async (event) => {
				...
				if (event.wasClean) {
					...
					if (tokenRefreshAttemptsRef.current >= maxTokenRefreshAttempts || timeSinceLastConnection < 5000) {
						console.log('Max token refresh attempts reached or immediate connection failure, not reconnecting')
						setConnectionStatus('disconnected')
						return
					}

					// Server closed connection cleanly - might be auth issue
					console.log(`WebSocket closed cleanly by server, attempting token refresh (${tokenRefreshAttemptsRef.current + 1}/${maxTokenRefreshAttempts})`)
				...
				console.log(`WebSocket disconnected unexpectedly, scheduling reconnection ${reconnectAttemptsRef.current}/${maxReconnectAttempts} in ${delay}ms`)
	...
	const disconnect = (reason = 'manual') => {
		...
		if (ws.current && ws.current.readyState === WebSocket.OPEN) {
			try {
				...
				if (reason === 'page_unload' || reason === 'page_hide' || reason === 'route_change') {
